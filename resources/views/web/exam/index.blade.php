...
@section('page_title', 'Thi - '.$course->name)
        ...
        <div class="test-page">
                    ...
                    <div class="scroll-on">
                        <ul class="nav-tabs">
                            @foreach ($list_question as $key => $value)
                                <li class="number-question questions {{ ++$key == 1 ? 'active' : '' }}" question-id="" total-number="50">
                                    <span class="question question-{{$key}}" data-id="#question_{{$value}}" data-url="{{route('w.exam.question',$value)}}">Câu {{$key}}</span>
                                </li>
                            @endforeach
                        </ul>
                    </div>
            ...
            <div class="content-body">
                <div class="quiz-wrapper bg-white rounded">
                    <ol>
                        <form action="{{route('w.exam.mark',['course' => $course,'exam_id' => $exam->id])}}" id="formExam" method="POST">
                            @csrf
...
<script>
    ...
    $(document).ready(function(){
        ...
        let numberQuestion = $('.question-1').text();
        function renderQuestion(questions, answers, numberQuestion){
            let question = `<h3> ${numberQuestion} </h3>`
        ...
        fetch(url).then(res => res.json()).then(
            (data)=>{
                renderQuestion(data[0], data[1],numberQuestion);
            }
        );

        // get info of the answer when user change questions
        $('.question').click(function(e){
            let numberQuestion = $(this).text();
